(() => {
  ...
  function statsHtml(data, teamNameValue) {
    ...
    const sortSelect = (options, selected) => `<label class="stats-sort-control"><span>Sort by</span><select data-stats-sort>${options.map(([key, label, optionDirection = 'desc']) => `<option value="${key}" data-direction="${optionDirection}"${key === selected ? ' selected' : ''}>${label}</option>`).join('')}</select></label>`;
    const battingOptions = [['runs', 'Runs'], ['average', 'Average'], ['strikeRate', 'Strike rate'], ['highScore', 'Highest score'], ['fifties', 'Fifties'], ['hundreds', 'Hundreds'], ['innings', 'Innings'], ['name', 'Player', 'asc']];
    const bowlingOptions = [['wickets', 'Wickets'], ['average', 'Average', 'asc'], ['economy', 'Economy', 'asc'], ['strikeRate', 'Strike rate', 'asc'], ['bestValue', 'Best figures'], ['overs', 'Overs'], ['innings', 'Innings'], ['name', 'Player', 'asc']];
    const fieldingOptions = [['catches', 'Catches'], ['stumpings', 'Stumpings'], ['runOuts', 'Run-outs'], ['name', 'Player', 'asc']];
    const battingTable = `<section class="stats-section" data-stats-view="batting"><div class="stats-heading"><div><span class="match-eyebrow">Batting</span><h3>Season batting</h3></div>${sortSelect(battingOptions, 'runs')}</div><div class="stats-table-wrap"><table class="stats-table" data-default-sort="runs"><thead><tr>${sortHead('Player', 'name')}${sortHead('Inn', 'innings')}${sortHead('Runs', 'runs')}${sortHead('HS', 'highScore')}${sortHead('Avg', 'average')}${sortHead('SR', 'strikeRate')}${sortHead('50', 'fifties')}${sortHead('100', 'hundreds')}</tr></thead><tbody>${batting.map((p) => `<tr>${statCell('Player', 'name', p.name)}${statCell('Innings', 'innings', p.innings)}${statCell('Runs', 'runs', p.runs, p.runs, true)}${statCell('Highest score', 'highScore', p.highScore, p.highScoreDisplay ?? p.highScore)}${statCell('Average', 'average', p.average)}${statCell('Strike rate', 'strikeRate', p.strikeRate)}${statCell('50s', 'fifties', p.fifties)}${statCell('100s', 'hundreds', p.hundreds)}</tr>`).join('')}</tbody></table></div></section>`;
    const bowlingTable = `<section class="stats-section" data-stats-view="bowling" hidden><div class="stats-heading"><div><span class="match-eyebrow">Bowling</span><h3>Season bowling</h3></div>${sortSelect(bowlingOptions, 'wickets')}</div><div class="stats-table-wrap"><table class="stats-table" data-default-sort="wickets"><thead><tr>${sortHead('Player', 'name')}${sortHead('Inn', 'innings')}${sortHead('Overs', 'overs')}${sortHead('Wkts', 'wickets')}${sortHead('BBI', 'bestValue')}${sortHead('Avg', 'average')}${sortHead('Econ', 'economy')}${sortHead('SR', 'strikeRate')}</tr></thead><tbody>${bowling.map((p) => { const parts = String(p.best || '0/9999').split('/').map(Number); const bestValue = (parts[0] || 0) * 10000 - (parts[1] || 9999); return `<tr>${statCell('Player', 'name', p.name)}${statCell('Innings', 'innings', p.innings)}${statCell('Overs', 'overs', p.overs)}${statCell('Wickets', 'wickets', p.wickets, p.wickets, true)}${statCell('Best figures', 'bestValue', bestValue, p.best)}${statCell('Average', 'average', p.average)}${statCell('Economy', 'economy', p.economy)}${statCell('Strike rate', 'strikeRate', p.strikeRate)}</tr>`; }).join('')}</tbody></table></div></section>`;
    const fieldingTable = fielding.length ? `<section class="stats-section" data-stats-view="fielding" hidden><div class="stats-heading"><div><span class="match-eyebrow">Fielding</span><h3>Season fielding</h3></div>${sortSelect(fieldingOptions, 'catches')}</div><div class="stats-table-wrap"><table class="stats-table stats-table-fielding" data-default-sort="catches"><thead><tr>${sortHead('Player', 'name')}${sortHead('Catches', 'catches')}${sortHead('Stumpings', 'stumpings')}${sortHead('Run-outs', 'runOuts')}</tr></thead><tbody>${fielding.map((p) => `<tr>${statCell('Player', 'name', p.name)}${statCell('Catches', 'catches', p.catches, p.catches, true)}${statCell('Stumpings', 'stumpings', p.stumpings)}${statCell('Run-outs', 'runOuts', p.runOuts)}</tr>`).join('')}</tbody></table></div></section>` : '';
    const note = teamNameValue === 'All'
      ? 'Combined from published 1st XI and 2nd XI scorecards. Players appearing for both teams are counted once.'
      : `Calculated from published ${esc(teamNameValue)} scorecards.`;
    return `<div class="stats-subtabs" role="tablist" aria-label="Statistics category"><button type="button" role="tab" data-stats-tab="batting" aria-selected="true">Batting</button><button type="button" role="tab" data-stats-tab="bowling" aria-selected="false">Bowling</button>${fielding.length ? '<button type="button" role="tab" data-stats-tab="fielding" aria-selected="false">Fielding</button>' : ''}</div><div class="season-stats">${battingTable}${bowlingTable}${fieldingTable}</div><p class="data-note">${note} Averages use recorded dismissals.</p>`;
  }

  function setupStatsTabs(root) {
    const tabs = [...root.querySelectorAll('[data-stats-tab]')];
    tabs.forEach((tab) => tab.addEventListener('click', () => {
      const panel = tab.dataset.statsTab;
      tabs.forEach((item) => item.setAttribute('aria-selected', String(item === tab)));
      root.querySelectorAll('[data-stats-view]').forEach((view) => { view.hidden = view.dataset.statsView !== panel; });
    }));
  }
  ...
  function setupStatsSorting(root) {
    root.querySelectorAll('.stats-section').forEach((section) => {
      ...
      section.querySelector('[data-stats-sort]')?.addEventListener('change', (event) => {
        const option = event.target.selectedOptions[0];
        sortRows(event.target.value, option.dataset.direction || 'desc');
      });
